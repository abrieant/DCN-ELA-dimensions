...
```{r read in csv file}
##Need to add script showing how we created this csv file (adapt from our jupyterhub script)
...
```{r code missing values}
#Replace 777 and 999 as na (none of our variables have 555 or 88 values)
...
```{r ecode education variable so that it is polytomous w/fewer categories}
...
#DEAP coding: 1: < HS Diploma | 2:HS Diploma/GED | 3: some college | 4: Bachelor |5: Post Graduate Degree
...
drop <- c("subjectkey")
ELAdata_touse_forFA_noNAs_noIDs <- ELAdata_touse_forFA_noNAs[,!(names(ELAdata_touse_forFA_noNAs) %in% drop)]

#write final data to csv
write.csv(ELAdata_touse_forFA_noNAs_noIDs, file = "ELAdata_noNAs_no_subjectIDs.csv", row.names = FALSE)
