...
###reading in the rs-fMRI data (that will be used for a subsequent study) and adjusting the subject IDs to match the other files
rsfMRI_subjects <- read.csv("/Users/hajernakua/Downloads/subjects_dcanderiv.csv")
rsfMRI_subjects$subjectkey <- sub("sub-", "", rsfMRI_subjects[,1]) 
rsfMRI_subjects$subjectkey <- sub("*NDAR", "NDAR_", rsfMRI_subjects[,1])

##merging subjects with ELA data and the rs-fMRI data 
ELAdata_wImaging <- merge(ELAdata, rsfMRI_subjects, by = "subjectkey")

##getting another data-frame with subjects without rs-fMRI data 
ELA_noImaging <- anti_join(ELAdata, ELAdata_wImaging, by = "subjectkey")

##check if there are spaces
rsfMRI_subjects$subjectkey <- trimws(rsfMRI_subjects$subjectkey, which = c("both"))
ELAdata$subjectkey <- trimws(ELAdata$subjectkey, which = c("both"))

##binding the rows of the two dataframes -> this will leave subjects with rs-fMRI data above those who don't 
ELAdata_corrected <- rbind(ELAdata_wImaging, ELA_noImaging)

#keep first participant from each family
ELAdata_corrected <- ELAdata_corrected[!duplicated(ELAdata_corrected$rel_family_id), ]
...
demo <- read_tsv(paste('/Users/hajernakua/Downloads/abcd_lt01.txt', sep='/'), col_names=TRUE) %>%
  filter(eventname == "baseline_year_1_arm_1") %>% 
...
```{r}
write.csv(ELAdata_forFA_noNAs, file = "ELAdata_noNAs.csv", row.names = FALSE)
```
