---
title: "Training Set - Multidimensional Scaling of all ELA Items"
...
saveRDS(ela_dist, "../results/trainset_ela_distance_matrix.RDS")
```
...
## 1 dimension
`stress = .28 ~ poor (risks for false interpretation) across both methods`
```{r}
mds1 <- isoMDS(ela_dist, y = cmdscale(ela_dist, 1), k = 1, maxit = 50, trace = TRUE,
       tol = 1e-3, p = 2)
mds1
saveRDS(mds1, "../results/trainset_ela_allitems_mds1.RDS")
...
# 2 dimensions
`stress = .15 ~ fair (some distances can be misleading) from both packages`
...
mds2 <- isoMDS(ela_dist, y = cmdscale(ela_dist, 2), k = 2, maxit = 50, trace = TRUE,
       tol = 1e-3, p = 2)
mds2
saveRDS(mds2, "../results/trainset_ela_allitems_mds2.RDS")
...
mds3 <- isoMDS(ela_dist, y = cmdscale(ela_dist, 3), k = 3, maxit = 50, trace = TRUE,
       tol = 1e-3, p = 2)
mds3
saveRDS(mds3, "../results/trainset_ela_allitems_mds3.RDS")
...
mds4 <- isoMDS(ela_dist, y = cmdscale(ela_dist, 4), k = 4, maxit = 50, trace = TRUE,
       tol = 1e-3, p = 2)
mds4
saveRDS(mds4, "../results/trainset_ela_allitems_mds4.RDS")
...
```{r}
#mds3_mono <- readRDS("../results/trainset_ela_allitems_mds3.RDS")
mds3_mono
mds3_mono$points
```
...
# save dataframe
write.csv(mds3_scores, "../results/trainset_mds3_allitems_scores.csv", row.names = F)
...
```{r}
jpeg(file = "../plots/all_items/mds3_trainset_dim1_dim2.jpeg", 
...
```{r}
jpeg(file = "../plots/all_items/mds3_trainset_dim1_dim3.jpeg", 
     width = 12, height = 6, units = "in", res = 1080)
...
```{r}
jpeg(file = "../plots/all_items/mds3_trainset_dim2_dim3.jpeg", 
     width = 12, height = 6, units = "in", res = 1080)
