---
title: "Full Sample - Multidimensional Scaling of all Child Report and Interview Items"
...
Load dataframes:
```{r}
df_wide <- read.csv("../data/ela_parent_report.csv", stringsAsFactors = F, header = T)
...
```{r}
cor_m <- cor(df_tr_scaled, method = "spearman")
head(cor_m)
```
...
saveRDS(ela_dist, "../results/ela_parentitems_distance_matrix.RDS")
```

```{r}
ela_vegdist <- vegdist(df_tr_scaled, method="gower", 
...
## 1 dimension
`stress = .35 ~ poor (risks for false interpretation) across both methods`
```{r}
mds1 <- isoMDS(ela_dist, y = cmdscale(ela_dist, 1), k = 1, maxit = 50, trace = TRUE,
       tol = 1e-3, p = 2)
mds1
saveRDS(mds1, "../results/ela_parentitems_mds1.RDS")
...
# 2 dimensions
`stress = .12 ~ fair (some distances can be misleading) from both packages`
...
mds2 <- isoMDS(ela_dist, y = cmdscale(ela_dist, 2), k = 2, maxit = 50, trace = TRUE,
       tol = 1e-3, p = 2)
mds2
saveRDS(mds2, "../results/ela_parentitems_mds2.RDS")
...
# 3 dimensions
`stress = .08 ~ good (can be confident in inferences from the plot)`
```{r}
mds3 <- isoMDS(ela_dist, y = cmdscale(ela_dist, 3), k = 3, maxit = 50, trace = TRUE,
       tol = 1e-3, p = 2)
mds3
saveRDS(mds3, "../results/ela_parentitems_mds3.RDS")
...
# 4 dimensions
`stress = .06 ~ good (can be confident in inferences from the plot)`
```{r}
mds4 <- isoMDS(ela_dist, y = cmdscale(ela_dist, 4), k = 4, maxit = 50, trace = TRUE,
       tol = 1e-3, p = 2)
mds4
saveRDS(mds4, "../results/ela_parentitems_mds4.RDS")
...
fl <- read.csv("../data/factor_loadings_parentitems.csv")
...
```{r}
#mds3_mono <- readRDS("../results/ela_parentitems_mds3.RDS")
mds3_mono
mds3_mono$points
```
...
# save dataframe
write.csv(mds3_scores, "../results/mds3_parentitems_scores.csv", row.names = F)
...
```{r}
jpeg(file = "../plots/parent_only/mds3_parentitems_dim1_dim2.jpeg", 
...
```{r}
jpeg(file = "../plots/parent_only/mds3_parentitems_dim1_dim3.jpeg", 
     width = 12, height = 6, units = "in", res = 1080)
...
```{r}
jpeg(file = "../plots/parent_only/mds3_parentitems_dim2_dim3.jpeg", 
     width = 12, height = 6, units = "in", res = 1080)
